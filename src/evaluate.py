"""
Stage 4b - Evaluation script.

Loads the best checkpoint, runs it on the held-out TEST split (speaker-
independent -- see dataset.split_dataset_by_speaker -- so these numbers
reflect performance on voices the model never saw during training), and
produces:
  - results/confusion_matrix.png
  - results/classification_report.txt
  - results/inference_latency.csv  (per-sample CPU inference time, for your
    real-time feasibility discussion)

Nothing here is fabricated: every number comes from actually running the
trained model on the test split.
...
def main():
    device = torch.device("cpu")  # evaluate on CPU: matches the real-time deployment target (Stage 5/6)
    ...
    if not test_samples:
        raise RuntimeError("Test split is empty. Check that data/commands/ is populated.")
    ...
    print("(This is model forward-pass time only -- Stage 5's realtime "
          "listener adds audio buffering/VAD overhead on top of this.)")
