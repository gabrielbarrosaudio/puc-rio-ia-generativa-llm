"""
Stage 4 - Training script.

Trains VoiceCommandCNN on YOUR recorded dataset, logs metrics every epoch to
results/train_log.csv, saves loss/accuracy curves as a PNG, and writes the
best checkpoint (by validation accuracy) to checkpoints/best_model.pt

Nothing here is fabricated: every number in train_log.csv comes from running
forward/backward passes over the clips you actually recorded in Stage 1.
...
def main():
    ...
    if not samples:
        raise RuntimeError(f"No data found in {config.DATA_DIR}. Run record_data.py first.")
