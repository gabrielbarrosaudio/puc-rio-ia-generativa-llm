"""
Stage 4 - Training script. Same as src/train.py and digits/train.py, plus
one addition: class-weighted loss (inverse frequency), as extra insurance
against class imbalance even after balancing the dataset composition
itself -- cheap to include, no downside if classes end up well balanced.
...
def compute_class_weights(train_samples, n_classes, device):
...
def main():
    ...
    if not samples:
        raise RuntimeError(f"No data found in {config.DATA_DIR}. Run prepare_dataset.py and extract_background_noise.py first.")
    train_samples, val_samples, _ = split_dataset_by_speaker(samples)
    print(f"Train: {len(train_samples)} | Val: {len(val_samples)} samples")

    counts = Counter(label for _, label in train_samples)
    print("Train class distribution:", {config.IDX_TO_LABEL[i]: counts.get(i, 0) for i in range(len(config.LABELS))})

    train_ds = VoiceCommandDataset(train_samples, augment=True)
    val_ds = VoiceCommandDataset(val_samples, augment=False)
    train_loader = DataLoader(train_ds, batch_size=config.BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=config.BATCH_SIZE, shuffle=False)

    model = VoiceCommandCNN().to(device)
    class_weights = compute_class_weights(train_samples, len(config.LABELS), device)
